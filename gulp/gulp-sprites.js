...
const IMG_DIST_PATH = 'assets/images/sprites/';
const CSS_DIST_PATH = 'assets/css/sprites/';
...
function createSpritesStream(name, src) {
  ...
  each(spritesheetSliceIndicies, (start, index) => {
    ...
    let spriteData = gulp.src(slicedSrc).pipe(
      spritesmith({
        imgName: `spritesmith-${name}-${index}.png`,
        cssName: `spritesmith-${name}-${index}.css`,
        algorithm: 'binary-tree',
        padding: 1,
        cssTemplate: 'sprites/raw_sprites/css/css.template.handlebars',
        cssVarMap
      })
    );
    ...
    let cssStream = spriteData.css.pipe(gulp.dest(CSS_DIST_PATH));
...
gulp.task('sprites:main', () => {
  let mainSrc = sync('sprites/raw_sprites/spritesmith/**/*.png');
  return createSpritesStream('main', mainSrc);
});

gulp.task('sprites:largeSprites', () => {
  let largeSrc = sync('sprites/raw_sprites/spritesmith_large/**/*.png');
...
gulp.task(
  'sprites:checkCompiledDimensions',
  gulp.series('sprites:main', 'sprites:largeSprites', done => {
...
gulp.task(
  'sprites:compile',
  gulp.series('sprites:clean', 'sprites:main', 'sprites:largeSprites', 'sprites:checkCompiledDimensions', done =>
    done()
  )
);
